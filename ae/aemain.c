#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>
#include <getopt.h>
#include <stdarg.h>
#include "evmain.h"
#include <netinet/in.h>
#include <ev.h>

#define DBG if(0) 
#define DBG_EV if(0) 

void accept_cb(struct ev_loop *loop, struct ev_io *watcher, int revents);
void read_cb(struct ev_loop *loop, struct ev_io *watcher, int revents);

struct settings settings;

typedef struct {
  char *start;
  char *write;
  char *read;
  unsigned long read_sz[2];
  char rhalf;
  char whalf;
  int sz;
  unsigned long last_wrap;
} slot_t;

slot_t slots[256][256] = {0};

typedef struct
{
  char *buf;
  int max_sz;
  int cur_sz;
  int needs;
} my_conn_t;

typedef struct
{
  ev_io base;
  my_conn_t *conn;
} my_ev_io_t;

struct sockaddr_in addr;

static int total_clients = 0;  // Total number of connected clients
static int total_mem = 0;
static int num_writes = 0;
static double start_time;



static void conn_init( my_conn_t *c ) {
  c->buf = malloc( 128*1024 );
  c->cur_sz = 0;
  c->max_sz = 128*1024;
  c->needs = 0;
}

static void conn_append( my_conn_t* c, char *data, int len ) {
  DBG printf(" append cur %d \n", c->cur_sz);
  if ( (c->cur_sz + len) > c->max_sz ) {
    while ( (c->cur_sz + len) > c->max_sz ) c->max_sz <<= 1;
    c->buf = realloc( c->buf, c->max_sz );
  }
  memcpy( c->buf + c->cur_sz, data, len ); 
  c->cur_sz += len;
}

static void print_buffer( char* b, int len ) {
  for ( int z = 0; z < len; z++ ) {
    printf( "%02x ",(int)b[z]);
  }
  printf("\n");
}

void on_data(my_conn_t *conn, ssize_t nread, char *buf) 
{

  DBG print_buffer( buf, 16 );

  if (nread <= 0) { //TODO
    //if (nread != UV_EOF) fprintf(stderr, "Read error %s\n", uv_err_name(nread));
    //uv_close((uv_handle_t*) client, on_close);
    //free(buf->base);
    return;
  }

  int data_left = nread;
  char *p;

  // If we have partial data for this connection
  if ( conn->cur_sz ) {
    DBG printf("Received %d more bytes need %d\n",(int)nread, conn->needs);
    //conn_print( conn );
    conn_append( conn, buf, nread );
    DBG print_buffer( conn->buf, 24 );
    //conn_print( conn );
    if ( conn->cur_sz >= conn->needs ) {
      p = conn->buf;
      conn->cur_sz = 0;
    } else {
      return;
    }
  } else {
    p = buf;
  }

  while ( data_left > 0 ) {

    if ( data_left < 4 ) {
      conn_append( conn, p, data_left );
      conn->needs = 4;
      DBG printf("Received partial data %d more bytes need %d\n",data_left, conn->needs);
      return;
    }

  int cmd   = (unsigned char)p[1];
  int topic = (unsigned char)p[2];
  int slot  = (unsigned char)p[3];
  DBG printf( " nread %d, data left %d cmd %d topic %d slot %d\n", (int)nread, data_left, cmd, topic, slot );

  if ( cmd == 9 ) {
    double taken = start_time/CLOCKS_PER_SEC;
    printf( " %d writes time taken %f \n ", num_writes, ((double)(clock()-start_time))/CLOCKS_PER_SEC );
    data_left -= 4;
    p += 4;
  }
  else if ( cmd == PUSH ) { 

    if ( data_left < 8 ) {
      conn_append( conn, p, data_left );
      conn->needs = 4;
      DBG printf("Received partial data %d more bytes need %d\n",data_left, conn->needs);
      return;
    }
    int data_sz   = *((int*)(p)+1);
    if ( data_sz > 24 ) {
      print_buffer(p, 24);
      exit(1);
    }

    DBG printf("data size %d\n", data_sz);
    if ( data_left < 8+data_sz ) {
      conn_append( conn, p, data_left );
      conn->needs = 8+data_sz;
      DBG printf("Received partial data %d more bytes need %d\n",data_left, conn->needs);
      DBG print_buffer( p, data_left);
      break; 
    }
    data_left -= 8 + data_sz;

    num_writes += 1;
    DBG printf( " %d writes nread %d left %d\n",num_writes,(int)nread,data_left);
    if ( num_writes == 100000 ) {
      start_time = clock();//time(NULL);
    }

    slot_t *s = &slots[topic][slot];

    if ( s->start == NULL ) {
      s->start = malloc( 4*1024*1024 );
      s->sz = 4*1024*1024;
      total_mem += s->sz;
      s->write = s->read = s->start;
      s->rhalf = 0; s->whalf = 0;
    }

    // Wrap
    if ( (s->start+s->sz)-(s->write+data_sz) < 0 ) { 
      int expand = 0;
      if ( data_sz > (s->sz>>3) ) expand = 1;
      if ( time(NULL) - s->last_wrap < 4 ) expand = 1; // Resize if we wrap too fast

      // Either we grow or wrap back to start
      if ( expand ) {
        int write = s->write-s->start;
        int orig = s->sz;
        DBG printf("expand data sz %d my sz %d\n", data_sz, s->sz);
        while ( data_sz > (s->sz>>3) ) {
          s->sz <<= 1;
          if ( total_mem + s->sz > settings.max_memory ) {
            fprintf( stderr, "Out of memory\n" ); // TODO Dump memory stats
            exit(1);
          }
        }

        if ( (total_mem+(s->sz-orig)) > settings.max_memory ) {
          if ( data_sz > orig ) {
            fprintf( stderr, "Out of memory\n" ); // TODO Dump memory stats
            exit(1);
          } 
        } else {  
          s->start = realloc( s->start, s->sz );
          s->write = s->start + write;
          DBG printf("Expanded to %d\n", s->sz);
        }

      } else {
        s->last_wrap = time(NULL);
        s->whalf ^= 1;
        s->write = s->start;
      }
    }

    // Halt if we bump into the read ptr          
    if ( s->rhalf != s->whalf ) {
      if ( s->write+data_sz >= s->read ) {
        //TODO drop the write
      } 
    }

    s->read_sz[s->whalf] += data_sz + 4;
    //printf( "YAY read sz[%d] %d\n", s->whalf, s->read_sz[s->whalf]);
    memcpy( s->write, p+4, data_sz+4 );
    p += 8 + data_sz;

  } 
  else if ( cmd == PULL ) { // pull

    data_left -= 4;
    p += 4;

    slot_t *s = &slots[topic][slot];
    // read s->read_sz[s->rhalf] bytes from the read ptr 
    //printf(" s->read_sz %d", s->read_sz[s->rhalf]);
    //write_req_t *req = (write_req_t*) malloc(sizeof(write_req_t));
    //req->buf = uv_buf_init(s->read, s->read_sz[s->rhalf]);
    //printf(" pull  %.*s\n", req->buf.len, req->buf.base );
    if ( s->read_sz[s->rhalf^1] != 0 ) {
      s->read_sz[s->rhalf] = 0;
      s->rhalf ^= 1;
    }
    //TODO uv_write((uv_write_t*) req, client, &req->buf, 1, on_write_done);

  } 
  else {
    printf( " bad cmd: %d topic %d slot %d\n", cmd, topic, slot );
    fprintf(stderr, "Received bad cmd %02x\n", cmd);
    exit(1);
  }
  }
     
}


static void sig_handler(const int sig) {
  double taken = start_time/CLOCKS_PER_SEC;
  printf( " %d writes time taken %f \n ", num_writes, ((double)(clock()-start_time))/CLOCKS_PER_SEC );
  //printf( " %d writes time taken %f \n ", num_writes, time(NULL)-start_time ); 
  printf("Signal handled: %s.\n", strsignal(sig));
  exit(EXIT_SUCCESS);
}

static void usage(void) {
  //printf(PACKAGE " " VERSION "\n");
  printf( "Mrq Version 0.1\n"
          "    -p, --port=<num>          TCP port to listen on (default: 7000)\n"
          "    -m, --max-socket=<megabytes>  Maximum amount of memory in mb (default: 256)\n"
          "\n"
        );
}
int main (int argc, char **argv) {

  signal(SIGINT, sig_handler);
  signal(SIGTERM, sig_handler);

  char *shortopts =
    "h"
    "m:"
    "p:"
    ;
  const struct option longopts[] = {
    {"help",             no_argument, 0, 'h'},
    {"max-memory", required_argument, 0, 'm'},
    {"port",       required_argument, 0, 'p'},
    {0, 0, 0, 0}
  };

  settings.port = 7000;
  settings.max_memory = 256;

  int optindex, c;
  while (-1 != (c = getopt_long(argc, argv, shortopts, longopts, &optindex))) {
    switch (c) {
    case 'p':
      settings.port = atoi(optarg);
      break;
    case 'm':
      settings.max_memory = atoi(optarg);
      break;
    case 'h':
      usage();
      return(2);
      break;
    default:
      usage();
      return(2);
    }
  }

  struct ev_loop *loop = ev_default_loop(0);
  int sd;
  struct sockaddr_in addr;
  int addr_len = sizeof(addr);
  struct ev_io w_accept;
  
  // Create server socket
  if( (sd = socket(PF_INET, SOCK_STREAM, 0)) < 0 )
  {
    perror("socket error");
    return -1;
  }
  
  bzero(&addr, sizeof(addr));
  addr.sin_family = AF_INET;
  addr.sin_port = htons(settings.port);
  addr.sin_addr.s_addr = INADDR_ANY;
  
  // Bind socket to address
  if (bind(sd, (struct sockaddr*) &addr, sizeof(addr)) != 0)
  {
    perror("bind error");
  }

  int enable = 1;
  setsockopt(sd, SOL_SOCKET, SO_OOBINLINE, &enable, sizeof(enable));
  
  // Start listing on the socket
  if (listen(sd, 128) < 0)
  {
    perror("listen error");
    return -1;
  }
  
  // Initialize and start a watcher to accepts client requests
  ev_io_init(&w_accept, accept_cb, sd, EV_READ);
  ev_io_start(loop, &w_accept);
  
  // Start infinite loop
  while (1)
  {
    ev_loop(loop, 0);
  }
  
  return 0;
}
  
  /* Accept client requests */
void accept_cb(struct ev_loop *loop, struct ev_io *watcher, int revents)
{
  //struct sockaddr_in client_addr;
  //socklen_t client_len = sizeof(client_addr);
  int client_sd;
  
  if(EV_ERROR & revents)
  {
    perror( "Error in accept" );
    return;
  }

  my_conn_t *conn = (my_conn_t*) malloc(sizeof(my_conn_t));
  conn_init(conn);
  my_ev_io_t *w_client = (my_ev_io_t*) malloc (sizeof(my_ev_io_t));
  w_client->conn = conn;
  //struct my_ev_io_t *w_client = (struct ev_io*) malloc (sizeof(struct ev_io));

  //TODO set no delay?
//TODO set rcv buff?
//int a = 65535;
//if (setsockopt(sockfd, SOL_SOCKET, SO_RCVBUF, &a, sizeof(int)) == -1) {
    //fprintf(stderr, "Error setting socket opts: %s\n", strerror(errno));
//}

  // Accept client request
  client_sd = accept(watcher->fd, NULL, NULL );// (struct sockaddr *)&client_addr, &client_len);
  
  if (client_sd < 0)
  {
    perror("accept error");
    return;
  }
  
  total_clients ++; // Increment total_clients count
  DBG_EV printf("Successfully connected with client.\n");
  DBG_EV printf("%d client(s) connected.\n", total_clients);
  
  // Initialize and start watcher to read client requests
  ev_io_init((ev_io*)w_client, read_cb, client_sd, EV_READ);
  ev_io_start(loop, (ev_io*)w_client);
}

#define BUFFER_SIZE 1024*1024
char recv_buffer[BUFFER_SIZE];

/* Read client message */
void read_cb(struct ev_loop *loop, struct ev_io *watcher, int revents){
  ssize_t nread;
  
  //if(EV_ERROR & revents)
  //{
    //perror("Invalid event in read_cb");
    //return;
  //}
  
  // Receive message from client socket
  nread = recv(watcher->fd, recv_buffer, BUFFER_SIZE, 0);
  
  if(nread < 0)
  {
    perror("read error");
    return;
  }
  
  if(nread == 0)
  {
    // Stop and free watcher if client socket is closing
    ev_io_stop(loop,watcher);
    free(watcher);
    perror("Peer EOF, closing");
    total_clients --; // Decrement total_clients count
    printf("%d client(s) connected.\n", total_clients);
    return;
  }

  my_conn_t* conn = ((my_ev_io_t*)watcher)->conn;
  on_data( conn, nread, recv_buffer );
  
  // Send message bach to the client
  //send(watcher->fd, buffer, read, 0);
  //bzero(buffer, read);
}
